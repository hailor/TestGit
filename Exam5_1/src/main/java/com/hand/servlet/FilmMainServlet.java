...
//		String sql = " select f.film_id,f.title,f.description,lan.name from film f join language lan on f.language_id = lan.language_id";
		String sql = " select c.first_name,c.last_name,a.address,c.email,c.customer_id,c.last_update from customer c join address a on c.address_id = a.address_id";
		try {
			...
			while(rs.next()){
				...
				map.put("last_update", rs.getDate("last_update"));
				list.add(map);
			}
