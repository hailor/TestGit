...
<div style="width: 95%;margin:0px auto">
    ...
    <form role="form" class="form-horizontal" action="<%=request.getContextPath()%>/UpdateServlet" method="post">
            ...
            <div class="col-sm-10">
                <select class="form-control" name="address_id">
                    <%
                        ...
                        while (rs.next()) {
                    %>
                    <option value="<%=rs.getInt("address_id")%>"
                            <%if ((rs.getInt("address_id")) == (Integer.parseInt(request.getAttribute("address_id").toString()))) {%>
                            selected="selected" <%}%>
        ...
        <input type="hidden" name="store_id" value="<%=request.getAttribute("store_id")%>">
        <input type="hidden" name="active" value="<%=request.getAttribute("active")%>">
        <input type="hidden" name="create_date" value="<%=request.getAttribute("create_date")%>">
